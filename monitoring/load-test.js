...
// Access URL function - revised version
function accessShortUrl() {
  ...
  const redirectRes = http.get(
    `http://localhost:3001/api/urls/${randomShortCode}`,
    {
      headers: {
        "Content-Type": "application/json",
  ...
  check(redirectRes, {
    "URL lookup successful": (r) => r.status === 200,
  }) || errorRate.add(1);

  if (redirectRes.status === 200) {
    try {
      const longUrl = JSON.parse(redirectRes.body).long_url;
      
      // Second request to actually visit the URL (optional, depending on your needs)
      const visitRes = http.get(longUrl);
      
      check(visitRes, {
        "URL visit successful": (r) => r.status === 200,
      }) || errorRate.add(1);
    } catch (e) {
